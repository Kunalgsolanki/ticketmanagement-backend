...
async function sendOtpEmail(toEmail, otp, userName = 'User') {
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      const deliveryEmail = !isProduction && testRecipient ? testRecipient : toEmail;

      if (deliveryEmail !== toEmail) {
        console.log(`[Mailer] Resend sandbox mode: routing OTP for ${toEmail} to ${deliveryEmail}.`);
      }

      console.log(`[Mailer] Sending OTP via Resend SDK to ${deliveryEmail}...`);
